export const initTetrisSocket = (io) => {
  ...
  // });
    ...
    socket.on('game start', () => { // 다른 누군가 게임 시작을 눌렀다면 다른 사용자들에게 알림
      ...
      garbageBlockCnt = [];
      rooms[room].players.forEach(player => garbageBlockCnt.push({id: player, live: true, garbageCnt: 0}));
    });
    ...
    socket.on('attack other player', garbage => {
      ...
      while(true) {
        if(garbageBlockCnt[idx].live) {
          break;
        }
        idx = (idx + 1) % garbageBlockCnt.length;
      }
    ...
    socket.on('game over', () => { // 누군가 게임 오버 시
      garbageBlockCnt[garbageBlockCnt.findIndex(player => player.id === socket.id)].live = false;
