...
export const initTetrisSocket = (mainSpace: Namespace, socket: userSocket) => {
  ...
  socket.on('attack other player', (garbage) => {
    ...
    if(target && target.garbageBlockCnt.length !== 1) { // 한 명 남은 경우 예외 처리
  ...
  socket.on('game over', () => {
    ...
    if(target) {
      ...
      target.rank.push({id: socket.id, oauthID: '', nickname: socket.userName, playTime: 0, ranking: target.gamingPlayer.length - target.gameOverPlayer, attackCnt: 0, attackedCnt: 0});
      target.gameOverPlayer++;

      if (target.gamingPlayer.length === 1 || target.gameOverPlayer === target.gamingPlayer.length - 1) {
  ...
  socket.on('get game over info', (oauthID, playTime, attackCnt, attackedCnt) => {
    ...
    if(target) {
      ...
      if(!rankTarget) {
        target.rank.push({id: socket.id, oauthID, nickname: socket.userName, playTime, ranking: 1, attackCnt, attackedCnt});
      }
      else {
      ...
      if(target.semaphore === target.gamingPlayer.length) {
        ...
        const date = new Date();
        const game_id = `${socket.roomID}${date.getDate()}`;
        const data = {
          'game' : {
            'game_id' : game_id,
