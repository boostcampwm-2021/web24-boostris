...
const moves = {
  ...
  [TETRIS.KEY.TURN_RIGHT]: (prev: Block) =>
    rotate(JSON.parse(JSON.stringify(prev)), true),
  [TETRIS.KEY.TURN_LEFT]: (prev: Block) =>
...
const initTetris = (BOARD: number[][], BLOCK: TetrisBlocks, STATE: TetrisState, TIMER: TetrisTimer, BACKGROUND: TetrisBackground) => {
...
const freezeBlock = (BOARD: number[][], BLOCK: TetrisBlocks, STATE: TetrisState, TIMER: TetrisTimer, option: string) => {
...
const initNewBlockCycle = (BOARD: number[][], BLOCK: TetrisBlocks, STATE: TetrisState, TIMER: TetrisTimer, BACKGROUND: TetrisBackground) => {
  BLOCK.NOW = BLOCK.NEXT;
...
const Board = ({
  ...
  useEffect(() => {
    ...
    BACKGROUND.IMAGE.onload = () => {
      ...
      TIMER.CONFLICT = setInterval(() => { // 블록이 떨어지면서 바닥에 도달한 경우, 0.5초 마다 움직임이 있는지 검사하는 타이머, 움직임이 없으면 freeze 있으면 다시 0.5초 동안 반복
      ...
      TIMER.SOLID_GARBAGE = setTimeout(() => setInterval(() => STATE.SOLID_GARBAGES++, 5000), 120000); // 솔리드 가비지 타이머
    };
